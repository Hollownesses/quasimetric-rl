...
def _batch_agent_value(
    ...
    values = [
        float(agent.value(obs_batch[i], goal_obs_batch[i]))
    ...
    *,
    ...
    if np.any(not_success):
        ...
        if float(cfg.alpha_final) != 0.0:
            ...
            costs[not_success] += float(cfg.alpha_final) * _batch_agent_value(agent, active_obs, goal_batch)
        ...
        if subgoal_obs is not None and float(cfg.alpha_subgoal) != 0.0:
            ...
            if np.any(need_subgoal):
                ...
                costs[need_subgoal] += float(cfg.alpha_subgoal) * _batch_agent_value(agent, terminal_obs_batch[need_subgoal], subgoal_batch)
